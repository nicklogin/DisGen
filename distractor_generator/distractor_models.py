...
def select_most_probable(row, answer_cols, inflect=False, k=4):
    outp = []
...
class DistractorModel(object):
    ...
    def postprocess_data(self, df):
        ...
        df = df[['Sentence','Right answer','Wrong answer','Distractor 1','Distractor 2', 'Distractor 3', 'Error type']]
...
class DualContextRNNModel(DistractorModel):
    ...
    def infer(self, df, answer_cols, inflect=False):
        ...
        model_output = get_k_predicted_items(self.model, DataIter, self.device, self.ANSWER.vocab.itos, k=4)
        ...
        df[['Distractor 1', 'Distractor 2', 'Distractor 3']] = df.apply(lambda x: select_most_probable(x, answer_cols, inflect),
                                                                        axis=1, result_type='expand')
...
class LexItemChoiceModel(DualContextRNNModel):
    def prepare_data(self, df):
        ## Filter:
        filter_func = lambda x: is_one_word(x['Right answer'], self.parser)
        df['Wrong answer'] = df['Sentence'].apply(find_error)
        df[['Keep 1','Right answer lemma', 'POS Right', 'Tag Right']] = df.apply(filter_func, axis=1,
                                                                                 result_type="expand")
        df[['Keep 2','Wrong answer lemma', 'POS Wrong', 'Tag Wrong']] = df.apply(filter_func, axis=1,
                                                                                 result_type="expand")

        df = df[(df['Keep 1'] == True) & (df['Keep 2'] == True)]
        verb_df = df[df['POS Right'] == 'VERB']
        verb_df['Tag'] = verb_df['Tag Right']
        verb_df = verb_df.drop(['Keep 1', 'Keep 2','POS Right','POS Wrong', 'Tag Right', 'Tag Wrong'], axis=1)


        ## Split context:
        verb_df[['left','right']] = verb_df.apply(lambda x: remove_err_span(x['Sentence']), axis=1,
                                                  result_type="expand")
...
class PrepositionsModel(DualContextRNNModel):
    ...
    def get_distractors(self, df):
        ...
        df = self.infer(df, answer_cols=('Right answer', 'Wrong answer'), inflect=False)
